...
/// WebIDL-shaped namespace: members are writable + configurable and not
/// enumerable, and `@@toStringTag` names the object.
const DEFINE_NAMESPACE_SHAPE: &str = r#"
(namespace, now, interfaces) => {
  ...
  // rquickjs constructors remain callable as functions; Temporal requires NewTarget.
  for (const name of interfaces) {
    const original = namespace[name];
    const wrapped = function (...args) {
      if (new.target === undefined) {
        throw new TypeError(name + " cannot be invoked without 'new'");
      }
  ...
  for (const name of interfaces) {
    tag(namespace[name]?.prototype, `Temporal.${name}`);
  }
...
pub mod temporal {
    ...
    pub fn evaluate<'js>(ctx: &Ctx<'js>, _: &Exports<'js>) -> Result<()> {
        ...
        ctx.eval::<Function, _>(crate::DEFINE_NAMESPACE_SHAPE)?
            .call::<_, ()>((namespace.clone(), now, interface_names))?;
        ctx.globals().set("Temporal", namespace)?;
        Ok(())
    }
}
